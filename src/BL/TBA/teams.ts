...
const getEventsStatuses = async (
  events: EventSimple[]
): Promise<Map<string, teamStatusInEvent>[]> => {
  const eventsStatueses: Map<string, teamStatusInEvent>[] = [];
  for (let i = 0; i < events.length; i += 100) {
    await new Promise((r) => setTimeout(r, 500));
    const chunk = events.slice(i, i + 100);
    const chunkStatuses = await Promise.all(
      chunk.map(async (event) => api.getEventStatuses(event.key))
    );
    eventsStatueses.push(
      ...(chunkStatuses.filter((a) => a != null) as Map<
        string,
        teamStatusInEvent
      >[])
    );
  }
  return eventsStatueses;
};
...
const teamsWinRate = async (year: string): Promise<TeamRecord[]> => {
  ...
  const eventsRankings: Map<string, teamStatusInEvent>[] = (
    await getEventsStatuses(events)
  )
...
export const topTeamsWinRate = async (year: string): Promise<string> => {
  ...
  return teams
    .map(
      (team, index) =>
        `${index + 1}. ${team.team.slice(3)} - wins:${
          team.record.wins
        }, loses:${team.record.losses}, ties:${team.record.ties}`
    )
    .join("\n");
};
...
export const topTeamsStats = async (year: string): Promise<string> => {
  ...
  const losses = JSON.parse(
    JSON.stringify(teams.sort((a, b) => b.record.losses - a.record.losses))
  ); // loses
  const wins = JSON.parse(
    JSON.stringify(teams.sort((a, b) => b.record.wins - a.record.wins))
  ); // wins
  const ties = JSON.parse(
    JSON.stringify(teams.sort((a, b) => b.record.ties - a.record.ties))
  ); // ties
  winrate.splice(10);
  losses.splice(10);
  wins.splice(10);
...
const getMatchRP = (match: Match, team: string): number => {
  if (
    ...
    (match.event_key.includes("2017") || match.event_key.includes("2016"))
  ) {
...
export const team4RPStats = async (
  team: string,
  year: string
): Promise<string> => {
  ...
  matches = matches.filter(
    (match) => match.comp_level == "qm" && (match.event_type ?? 0) <= 5
  );
  return `${
    (matches.filter((match) => getMatchRP(match, team) == 4).length /
      matches.length) *
    100
  }%\n
